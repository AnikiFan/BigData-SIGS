...
* 服务器介绍
  ...
  - 集群一：ip地址： 10.103.9.11 ，可用机器：01，03，04，07。登录集群一01的命令： ssh [xxx@10.103.9.11](mailto:xxx@10.103.9.11) （也就是和以下实验指导书的内容完全相同）
...
请编写两个 shell 脚本，分别在 4 个节点（如集群一的 thumm01、03、04、07）上同时启动/停止一个示例进程（推荐用 `sleep 600` 模拟）：
...
2. **Distribute（分发到各节点）**：
    ```shell
    NODES=(thumm01 thumm03 thumm04 thumm07)
