...
......
2021214323:x:1372:502::/home/dsjxtjc/2021214323:/bin/bash
2021280795:x:1373:502::/home/dsjxtjc/2021280795:/bin/bash
```
...
```shell
2019211199@thumm01:~$ awk -F: '$1~"^2021"{print $1}' /etc/passwd
2021210991
2021211018
......
20212144842
...
其中-F:参数表示使用`:`作为分隔符进行分割，\$1~"^2021"{print \$1}中\$1表示分割后第一个元素（用户名），"\^2021"是一个正则表达式，表示以2021开头，`~`表示匹配，所以\$1\~"^2021"表示分割后第一个元素满足2021开头，那么就执行后面的指令{print \$1}。

使用`wc -l`统计下有多少个这样的学号，即可知道参加本课程的21级同学的数量了。

```shell
2019211199@thumm01:~$ awk -F: '$1~"^2021"{print $1}' /etc/passwd | wc -l
xx
```
...
4. 全部启动完成后，再次 `ssh` 各节点用 `pgrep -x sleep` 验证进程已经存在，并把结果汇总打印。

`stop_all.sh` 应该使用 `pkill -x sleep` 或 `kill <pid>` 把刚才启动的进程清理掉，并打印每台机器的清理结果。

> **为什么这里用 `-x` 而不是 `-f`**：`-f` 会让 `pgrep`/`pkill` 拿**整条命令行**去匹配，而不只是进程名。`ssh <node> "... pkill -f 'sleep 600' ..."` 在远端是由一个 shell 执行的，这个 shell 自己的命令行里就含有 `sleep 600` 这段文字——只要它还要继续执行后面的语句（因而没有被 `exec` 掉），它就会被自己的 `pkill` 命中：你会看到退出码 143（128 + SIGTERM）而不是预期的 0，后面的语句也不会再执行。同理，`pgrep -f "sleep 600"` 会把这个 shell 当成一个"还在运行的进程"报出来，让你误以为没有停干净。
>
> `-x sleep` 表示精确匹配进程名 `sleep`，不看参数，因此不会匹配到自身。代价是它会命中你所有的 `sleep` 进程（包括任务 5 里可能还没退出的那个），所以启动前先确认没有别的 `sleep` 在跑。
>
> 如果你确实需要按参数区分进程，推荐启动时把 PID 写进文件（`echo $! > /tmp/demo.pid`），停止时按 PID 精确 `kill`——这也更接近 lab2 / lab4 里管理 NameNode、DataNode 的真实做法。另一种常见技巧是让模式避开自身，例如 `pgrep -f "[s]leep 600"`（正则 `[s]leep` 能匹配 `sleep`，但匹配不到命令行里 `[s]leep` 这串字面文本），不过它只能保护"以带方括号形式写有该模式"的那一层，外层包装如果又出现了未加方括号的原文，自匹配还是会回来。
