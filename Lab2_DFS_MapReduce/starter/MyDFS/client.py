...
class Client:
    ...
    def mkdir(self, dfs_path):
        request = "mkdir {}".format(dfs_path)
        print("Request: {}".format(request))
        # TODO: 向NameNode发送mkdir请求，接收并打印返回结果

    def mv(self, src_path, dst_path):
        request = "mv {} {}".format(src_path, dst_path)
        print("Request: {}".format(request))
        # TODO: 向NameNode发送mv请求，接收并打印返回结果
...
if cmd == '-ls':
    ...
    if argc == 2:
        dfs_path = argv[2]
        client.mkdir(dfs_path)
    else:
        print("Usage: python client.py -mkdir <dfs_path>")
elif cmd == "-mv":
    if argc == 3:
        src_path = argv[2]
        dst_path = argv[3]
        client.mv(src_path, dst_path)
    else:
        print("Usage: python client.py -mv <src_path> <dst_path>")
    ...
    print("Usage: python client.py <-ls | -mkdir | -mv | -copyFromLocal | -copyToLocal | -rm | -format> other_arguments")
